...
const Flightcheckout = () => {
  ...
  useEffect(() => {
    const userdata = getUserData("safarix_user");
    if (!userdata) {
  ...
  // ✅ UNIQUE KEY PER TRACEID (VERY IMPORTANT)
  const FLIGHT_CHECKOUT_STORAGE_KEY = "flight_checkout_data";
  ...
  useEffect(() => {
    try {
      ...
      if (parsed.passengerConfirmed) setPassengerConfirmed(parsed.passengerConfirmed);
  ...
  useEffect(() => {
    ...
    // ✅ CRITICAL FIX: DO NOT overwrite restored passengers
    if (isRestored) {
      console.log("Passengers restored, skipping initializePassengers");
      return;
    }

    console.log("Initializing passengers fresh");
  ...
  }, [state, restoreChecked, isRestored]);
  ...
  const handleResetFormData = () => {
    if (!window.confirm("Are you sure you want to reset passenger form data?")) return;

    initializePassengers(state?.passengerCount || { adults: 1, children: 0, infants: 0 });
    setContactEmail("");
    setContactMobile("");
    setGstDetails({ gstNumber: "", companyName: "", companyAddress: "", companyEmail: "" });
    setPassengerConfirmed(false);
          ...
          <div className="d-flex justify-content-between align-items-center">
            <div>
              ...
              <small className="text-muted">
                {formatDate(flightInfo?.origin.time)} •{" "}
                {flightInfo?.airline.name} • Flight{" "}
  ...
  const validatePassengers = () => {
    ...
    for (let i = 0; i < passengers.length; i++) {
      ...
      if (!/^[0-9]{10}$/.test(p.contactNo)) {
        alert(
          `Contact number must be exactly 10 digits for ${p.type} ${i + 1}`,
        );
        return false;
      }
  ...
  return (
    <>
        ...
        <Row>
          <Col lg={8}>
              ...
              <Card.Header
                onClick={() => {
                  ...
                  if (
                    ...
                    setSelectedBaggageList(
                      Object.values(
                        selectedBaggage[activeFlightIndex] || {},
                      ).filter(Boolean),
                    );
                  }
                ...
                <span className="text-end">
                  {selectedBaggageList.length > 0 ? (
                    <>
                      ...
                      <small className="text-muted d-block">
                        {selectedBaggageList.length} of {requiredSeatCount} selected
                      </small>
                      <div className="mt-1" style={{ fontSize: "12px" }}>
                        {baggageSummaryByFlight.map((f, idx) => (
                          <div key={idx}>
                            <div className="fw-semibold text-primary">{f.route}</div>
                            {f.items.map((t, i) => (
                              <div key={i} className="ms-2 text-muted d-flex align-items-center gap-1">
                                • {t}
                                <span
                ...
                <Card.Body>
                  ...
                  <div className="mb-3">
                    ...
                    <div className="progress mt-1" style={{ height: "6px" }}>
                      <div
                        ...
                        style={{
                          width: `${(selectedBaggageList.length / requiredSeatCount) *
                            100
                            }%`,
                        }}
                      />
                  ...
                  {ssrDataArray[activeFlightIndex]?.baggage?.map((bag, i) => {
                    ...
                    return (
                      <div
                        key={i}
                        className={`ssr-option p-3 mb-2 rounded border ${isSelected ? "border-primary bg-light" : ""
                          }`}
            ...
            <Card className="mb-3">
              ...
              >
                ...
                <span className="text-end">
                  {Object.keys(selectedSeats[activeFlightIndex] || {}).length > 0 ? (
                    <>
                      <Badge bg="success" className="me-2">✓</Badge>
                      <small className="text-muted d-block">
                        {Object.keys(selectedSeats[activeFlightIndex] || {}).length} seat(s) selected
                      </small>
                      <div className="mt-1 text-primary" style={{ fontSize: "12px" }}>
                              ...
                              <div key={i} className="ms-2 text-muted d-flex align-items-center gap-1">
                                ...
                                <span
                                  ...
                                  onClick={(e) => {
                                    ...
                                    const flightIndex = Object.keys(selectedSeats).find(
                                      (fi) => selectedSeats[fi]?.[i]?.Code === s.seat
                                    );
                                    if (flightIndex !== undefined) {
                                      setSelectedSeats((prev) => {
                                        const flightSeatMap = { ...(prev[flightIndex] || {}) };
                                        delete flightSeatMap[i];
                                        return { ...prev, [flightIndex]: flightSeatMap };
                                      });
                                      setSelectedSeatList((prev) => prev.filter((seat) => seat.Code !== s.seat));
                                    }
                                  }}
                                >
                                  ✕
          ...
          <Col lg={4}>
            ...
            <Card className="sticky-top shadow-sm" style={{ top: "98px" }}>
              ...
              <Card.Body>
                {fareSource === "search" && (
                  <div className="fare-loading-block text-center py-4 border rounded bg-light">
                    <Spinner animation="border" />

                    <div className="mt-3 fw-semibold">
                      Confirming fare with airline...
                    </div>
                        ...
                        <>
                          <div className="insurance-plans mb-3">
                            {insuranceData.map((plan, index) => (
                              <div
                                key={index}
                                className={`insurance-plan p-2 mb-2 border rounded ${selectedInsurancePlan?.PlanCode ===
                                  plan.PlanCode
                                  ? "border-primary"
                                  : ""
                                  }`}
                ...
                {selectedSeatList.length > 0 && (
                  <div className="text-end mb-2">
                    <Button
                      ...
                      onClick={() => {
                        setSelectedSeatList([]); // clear modal local state
                        setSelectedSeats((prev) => ({
                          // ✅ also clear persistent state immediately
                          ...prev,
                          [activeFlightIndex]: {},
                        }));
                      }}
                    ...
                    {row.Seats.map((seat, seatIndex) => {
                      ...
                      return (
                        <React.Fragment key={`${seat.Code}-${seatIndex}`}>
                          <button
                            className={`seat-button ${!isAvailable
                              ? "seat-occupied"
                              : isSelected
                                ? "seat-selected"
                                : seat.Price === 0
            ...
            return (
              <>
                ...
                <Button
                  ...
                  // }}
                  onClick={() => {
                    // Build a new seat map: paxIndex → seat (only if seat exists)
                    const finalSeatMap = {};
                    seatEligiblePassengers.forEach((pax, index) => {
                      if (selectedSeatList[index]) {
                        finalSeatMap[index] = selectedSeatList[index];
                      }
                      // If no seat at this index, we intentionally omit it (= deselected)
                    });
                ...
                >
                  {selectedSeatList.length < requiredSeatCount
                    ? `Select ${requiredSeatCount - selectedSeatList.length
                    } more seat(s)`
                    : "Done"}
                </Button>
              </>
      ...
      {/* Confirmation Modal */}
      <Modal show={showConfirmModal} centered backdrop="static" size="lg">
        <Modal.Header>
          <Modal.Title>Confirm Booking & Payment</Modal.Title>
        </Modal.Header>
        ...
        <Modal.Body>
          {loading ? (
            ...
            <>
              ...
              <div className="mb-3">
                ...
                <div className="border rounded p-3">
                  <div className="d-flex justify-content-between mb-1">
                    ...
                    <span>{selectedFlights.length} Flight(s)</span>
                  </div>

                  <div className="d-flex justify-content-between mb-1">
                    <span className="text-muted">Passengers</span>
                    <span>
                      {passengerCount.adults} Adult(s),{" "}
                      {passengerCount.children} Child(s),{" "}
                  ...
                  <div className="d-flex justify-content-between">
                    <span className="text-muted">Travel Class</span>
                    <span>{state?.travelClass || "Economy"}</span>
                  </div>
                </div>
              </div>
              {/* FLIGHT DETAILS */}
                  ...
                  return (
                    <div key={index} className="border rounded p-2 mb-2">
                      <div className="fw-semibold">
                        {info.airline.name} ({info.airline.code}
                        {info.airline.flightNumber})
                      </div>

                      <div className="small text-muted">
                        {info?.origin.city} ({info?.origin.code}) →{" "}
                        {info?.destination.city} ({info?.destination.code})
                      </div>

                      <div className="small text-muted">
                        {formatDate(info?.origin.time)} •{" "}
                        {formatTime(info?.origin.time)} →{" "}
                        {formatTime(info?.destination.time)}
                      </div>
                    </div>
                  );
                })}
              ...
              {/* BAGGAGE DETAILS */}
              {baggageSummaryByFlight.length > 0 && (
                <div className="mb-3">
                  <h6 className="fw-bold">Extra Baggage</h6>

                  {baggageSummaryByFlight.map((flight, idx) => (
                    <div key={idx} className="border rounded p-2 mb-2">
                      <div className="fw-semibold">{flight.route}</div>
              ...
              {/* {Object.keys(selectedMeals).length > 0 && (
                <div className="mb-3">
                  ...
                  {Object.entries(selectedMeals).map(([flightIndex, meals]) => {
                    const info = extractFlightInfo(
                      selectedFlights[flightIndex],
                    );
              ...
              <span>
                {Object.values(selectedMeals).some(m => Object.values(m || {}).some(Boolean)) ? (
                  <>
                    <Badge bg="success" className="me-2">✓</Badge>
                    <div style={{ fontSize: "12px" }}>
                      {selectedFlights.map((flight, flightIndex) => {
                        ...
                        const mealItems = Object.entries(mealsForFlight).filter(([, m]) => m);
                        ...
                        return (
                          <div key={flightIndex}>
                            ...
                            {mealItems.map(([paxIndex, meal]) => (
                              <div key={paxIndex} className="ms-2 text-muted d-flex align-items-center gap-1">
                                • Pax {Number(paxIndex) + 1}: {meal.AirlineDescription}
                                <span
                                  className="text-danger"
                                  style={{ cursor: "pointer", fontWeight: "bold" }}
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    setSelectedMeals((prev) => {
                                      const flightMeals = { ...(prev[flightIndex] || {}) };
                                      delete flightMeals[paxIndex];
                                      return { ...prev, [flightIndex]: flightMeals };
                                    });
                                    setSelectedMealList((prev) => prev.filter((_, i) => String(i) !== paxIndex));
                                  }}
                                >
                                  ✕
                                </span>
              ...
              {selectedInsurancePlan && (
                <div className="mb-3">
                  <h6 className="fw-bold">Insurance Plan</h6>
                  ...
                  <div className="border rounded p-2">
                    ...
                    <div className="small text-muted">
                      Coverage ₹{" "}
                      {Number(
                        selectedInsurancePlan.SumInsured,
                      ).toLocaleString()}
                    </div>
              ...
              {priceChanged && (
                <Alert variant="warning">
                  Fare increased by ₹ {formatPrice(fareDiff)}
                </Alert>
              )}
