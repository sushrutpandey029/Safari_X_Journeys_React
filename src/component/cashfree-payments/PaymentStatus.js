...
export default function PaymentStatus() {
  ...
  const [message, setMessage] = useState("");
  const [bookingId, setBookingId] = useState(null);
  const [countdown, setCountdown] = useState(5); // redirect counter
  ...
  // Auto redirect if success → go user dashboard
  useEffect(() => {
    if (status === "success") {
      ...
      return (
        <div className="text-green-600">
          ...
          <p className="mt-3 text-green-700 font-semibold">
            Redirecting to your dashboard in <strong>{countdown}</strong>{" "}
            seconds...
          </p>
        </div>
      );
